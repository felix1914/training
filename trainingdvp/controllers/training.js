...
module.exports.GetAllDailyUpdates = async (req, res) =>{
    let err, dailyUpdates
    [err, dailyUpdates] = await to(DailyUpdates.find({active:true}));
    if(err)return ReE(res, err, HttpStatus.INTERNAL_SERVER_ERROR)
    return ReS(res, {
...
module.exports.GetByDepartment = async (req, res) =>{
    let err, dailyUpdates
    [err, dailyUpdates] = await to(DailyUpdates.find({active:true,department:req.user.department}));
    if(err)return ReE(res, err, HttpStatus.INTERNAL_SERVER_ERROR)
    return ReS(res, {
        message: 'DailyUpdates Founded',
        dailyUpdates:dailyUpdates,
...
module.exports.DeleteDailyUpdates = async (req, res) =>{
    ...
    if(!dailyUpdates){
        return ReE(res,
            {message: 'DailyUpdates not found1'},
